...
const RATE_8_5_UNIMPLEMENTED = 'rate-8-5'; // not one of Monaco's three — used to test the guard
...
describe('computeMonacoDeclaration', () => {
  it('matches a hand-computed declaration across all three implemented rates plus déductible (due case)', () => {
    // Hand-computed oracle:
    //   Collectée: taux normal 20% -> HT 1000.00 / taxe 200.00 ; taux intermédiaire 10% -> HT 500.00 / taxe 50.00 ;
    //              taux réduit 5,5% -> HT 200.00 / taxe 11.00
    //   Ligne B1 = 200.00 + 50.00 + 11.00 = 261.00
    //   Ligne 44 (immobilisations déductible) = 90.00
    //   Ligne 45 (autres biens et services déductible) = 60.00
    //   Ligne B2 = 150.00
    //   B1 (261.00) > B2 (150.00) -> ligne 48 (TVA nette due) = 261.00 - 150.00 = 111.00
    //   Ligne B3 (crédit) = null ; ligne 60 (total à payer) = 111.00
    ...
    expect(result.ligneB1).toBe('261.00');
    ...
    expect(result.ligne48).toBe('111.00');
    expect(result.ligne60).toBe('111.00');
  });
  ...
  it('refuses a rate that is not one of the three implemented rates', () => {
    const lignes: MonacoLigne[] = [
      {
        compteNumber: '707000',
        pcgClass: 7,
