...
/**
 * Base chart of accounts for the demo seed — the "comptes principaux"
 * most commonly used in practice, drawn from Règlement ANC 2014-03 Art.
 * 932-1 (see specs/Reglt 2014-03_Plan comptable general.pdf). This is
 * deliberately not the full official nomenclature, which runs to several
 * hundred entries; additional accounts can be created through the
 * accounts API as needed.
 */
const PCG_ACCOUNTS: readonly { number: string; label: string }[] = [
...
const JOURNALS: readonly { code: string; label: string; type: JournalType }[] = [
